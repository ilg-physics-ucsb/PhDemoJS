...
//Initial Values
var S=0.25, W=0.04, lambda=0.000650
var diff=true 
var inf=true
...
isdiff.checked=true
var isinf=document.getElementById("Interference")
isinf.checked=true
...
//Diffraction
function I_diffract(S,W,theta,lambda){   
  return sinc(Math.PI*W*Math.sin(theta)/lambda)**2
  }
  
//Interference
function I_interfere(S,W,theta,lambda){   return (Math.cos(Math.PI*S*Math.sin(theta)/lambda)**2)
  }
...
function color_the_light(){
 ...
 light_color_val= Math.max(Math.round((0.000700-lambda)/(visible_range)*250)-50,0)
...
var trace_inf= {
    ...
    mode: 'lines',
    marker: {color: ['red']},
    line: {'shape': 'spline', 'smoothing': 2, color: 'rgba(194, 194, 194,1)'}
}
var trace_dif= {
    x: x_grid,
    y: I_dif,
    ...
    mode: 'lines',
    marker: {color: ['green']},
    line: {'shape': 'spline', 'smoothing': 2, color:'rgba(107, 107, 107,1)'}
    
}

var traces_all=[trace_inf, trace_dif, trace_pat]
var data = [
  {
    z: I_y,
    x: x_grid,
...
var layout_demo2={
  ...
  Plotly.newPlot('graph', 
      [trace_inf, trace_dif, trace_pat] , layout_graph,{displayModeBar: false}
  )
...
function initialize(s,w,l){
  ...
  if (diff) {
    trace_dif.line={'shape': 'spline', 'smoothing': 2, color:'rgba(107, 107, 107,1)'}
  }else{
    trace_dif.line={'shape': 'spline', 'smoothing': 2, color:'rgba(107, 107, 107,0)'}
  }
  if (inf) {
    trace_inf.line={'shape': 'spline', 'smoothing': 2, color:'rgba(194, 194, 194,1)'}
  }else{
    trace_inf.line={'shape': 'spline', 'smoothing': 2, color:'rgba(194, 194, 194,0)'}
  }
  ...
  for (i=0; i<steps;i++){
   ...
   I_inf[i]=I_interfere(S,W,th,lambda)
   I_dif[i]=I_diffract(S,W,th,lambda)
  }
  //Compute Heatmap
  for (i=0; i<steps;i++){
  ...
  Plotly.animate('graph', 
    {
     traces: [trace_inf, trace_dif, trace_pat]
     }, 
    {
      transition: 
        {
